...
window.createGuide = function ({T, scene, player, ally, state, blocked, floorAt, move, toast}) {
  const stops=[
  ...
  guide.reset=()=>{Object.assign(guide,{stage:0,complete:false,status:'准备出发',path:[],timer:0});marker.visible=false;};
  guide.update=function(dt) {
    ...
    if(d>17||guide.returning&&d>5){guide.returning=true;guide.status='返回接应';return walk(player,dt,4.5);}
